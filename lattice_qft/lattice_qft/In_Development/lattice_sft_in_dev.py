...
class sft_lattice(Lattice):
    ...
    def Gij_matrix(self):  # SFT
        ...
        x, y = self.x_list()._grid
        xp, yp = self.p_list()._grid
        xo, yo = self.omega_list()
        pos_coords =[]
        m_coords = []
        o_coords = []
        for i in range(self._nL):
            for j in range(self._nL):
                o_coords.append([xo[i][j],yo[i][j]])
        for i in range(self._nL):
            for j in range(self._nL):
                pos_coords.append([x[i][j],y[i][j]])
        for i in range(self._nL):
            for j in range(self._nL):
                m_coords.append([xp[i][j],yp[i][j]])        
        for i in range(self._nL*self._dimension):
            for j in range(self._nL*self._dimension):
                explist = np.exp(np.array(m_coords)
                            *(np.array(pos_coords)[i] - np.array(pos_coords)[j])*1.0j
                    )
                # Should be times dx in the equation below!
                correlation[i, j] = np.real(np.sum(((o_coords)*explist))**2
                    * (self._dx) / self._nL
                )
        return correlation
...
#Lattice()
lat = sft_lattice(2, 4, 1, 1, 1)
lr = lat.get_q_register() 
testqc = QuantumCircuit(lr)
import scipy


lat.ground_state(testqc,lr)
print(testqc.draw())
