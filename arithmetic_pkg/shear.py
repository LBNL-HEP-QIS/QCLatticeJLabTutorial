    ...
    if r > 0:
                ...
                if A[j][k+r-n-1] == '1': # Check if the nth bit of a_ij is one
                    if n <= r:
                        if n == 0:
                            ...
                            qc.compose(RippleAdder(k+r, modular=True, circuit=False), anc[:] + nDic[j+i+1][:] + ext[:r] + dec[:r] + nDic[i][:], inplace=True) # k + m - n
                            ...
                            qc.compose(RippleAdder(k+r-n+1, modular=True, circuit=False), anc[:] + ext[-1:] + nDic[j+i+1][:] + ext[:r-n] + dec[n-1:r] + nDic[i][:], inplace=True)
                    else:
                        if n == k + r - 1: # subtract
                            qc.compose(Subtractor2(k+r-n+1, modular=True, circuit=False), anc[:] + nDic[i][n-r-1:] + ext[-1:] + nDic[j+i+1][:k+r-n], inplace=True)
                        else:
                            qc.compose(RippleAdder(k+r-n+1, modular=True, circuit=False), anc[:] + ext[-1:] + nDic[j+i+1][:k+r-n] + nDic[i][n-r-1:], inplace=True) # k + m - n
        ...
        for j in reversed(range(S-i-1)):
            ...
            for n in reversed(range(r+1)):
                if A[j][k+r-n-1] == '1': # Check if the nth bit of a_ij is one
                    if n == r:
                        qc.compose(RippleAdder(r-n+1, modular=True, circuit=False).reverse_ops(), anc[:] + ext[-1:] + dec[n-1:r], inplace=True)
                    elif r - k >= n:
                        if n == 0:
                            qc.compose(RippleAdder(r, modular=True, circuit=False).reverse_ops(), anc[:] + nDic[j+i+1][:] + ext[:r-n-k] + dec[:], inplace=True) # m - n
                        else:
                            qc.compose(RippleAdder(r-n+1, modular=True, circuit=False).reverse_ops(), anc[:] + ext[-1:] + nDic[j+i+1][:] + ext[:r-n-k] + dec[n-1:r], inplace=True) # m - n
                    else:
                        qc.compose(RippleAdder(r-n+1, modular=True, circuit=False).reverse_ops(), anc[:] + ext[-1:] + nDic[j+i+1][:r-n] + dec[n-1:r], inplace=True) # m - n
        ...
        for j in range(S-i-1):
            for n in range(k):
                if A[j][k-n-1] == '1': # Check if the nth bit of a_ij is one
                    qc.compose(RippleAdder(k-n, modular=True, circuit=False), anc[:] + nDic[j+i+1][:k-n] + nDic[i][n:], inplace=True) # k + m - n
